...
[[nodiscard]] auto load_font(std::string &&font_file_path) -> bool {
  ...
  for (GLubyte c = 0; c < 128; ++c) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, freetype_face->glyph->bitmap.width,
                 freetype_face->glyph->bitmap.rows, 0, GL_RED, GL_UNSIGNED_BYTE,
    ...
    Character character{
        texture, glm::ivec2(freetype_face->glyph->bitmap.width, freetype_face->glyph->bitmap.rows),
...
auto main(int argc, char **argv) -> int {
  ...
  const auto render_text = [&](std::string &&text, GLfloat x, GLfloat y, GLfloat scale,
                               glm::vec3 color) {
    shader.Use();
